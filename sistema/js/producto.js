$(document).ready(function () {
  ...
  $("#frmChangePass").submit(function (e) {
    ...
    if (passNuevo.length < 5) {
      $(".alertChangePass").html(
        '<p style="color:orangered;">Las contraseñas deben contener como mínimo 5 caracteres'
      );
      $(".alertChangePass").slideDown();
      return false;
    }
...
function validaPass() {
  ...
  $(".alertChangePass").html(
    '<p style="color:blue;">Las contraseñas Coinciden.</p>'
  );
  $(".alertChangePass").slideDown();
}
...
function delProduct() {
  ...
  $.ajax({
    ...
    success: function (response) {
      if (response == "error") {
        $(".alertAddProduct").html(
          '<p style="color : red;">Error al eliminar producto.</p>'
        );
      } else {
        $(".row" + pr).remove();
